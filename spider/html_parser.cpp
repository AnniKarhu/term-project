...
std::set<std::string> html_parser::get_urls_from_html(const std::string& html_body_str, const std::string& base_str)
{
    ...
    1. Ќайти тег <title></title> - забрать слова из середины, можно регул€рными выражени€ми
    2. Ќайти тег <body >. Ќайти - сначала позицию <body, или Ѕ < body> - удалить всю строку до этого тега
    3. функци€ поиска любого тега <> - первого попавшегос€ в начале строки - по поиску std::string find - поиск < затем поиск > затем снова поиск <
    4. ‘ункци€ определени€ типа тега по именм. 
    
        цикл:
            5. ѕоиск следующего тега - если a - отправить на парсинг дл€ получени€ ссылки с регул€рными выражени€ми
            6. удалить тег
        конец цикла
        
    7. удалить все повтор€ющиес€ пробелы, табул€ции, знаки препинани€, переводы строки
        8. ѕеревести все в нижний регистр
        9. —кормить строку в map с подсчетом повторений слов
